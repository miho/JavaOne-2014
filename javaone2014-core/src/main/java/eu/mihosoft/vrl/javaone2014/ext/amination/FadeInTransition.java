package eu.mihosoft.vrl.javaone2014.ext.amination;
...
public class FadeInTransition extends CachedTimelineTransition {
     ...
     */
    public FadeInTransition(final Node node, double preTime, double time) {
        super(
            node,
            TimelineBuilder.create()
                .keyFrames(
                    new KeyFrame(Duration.millis(0),    new KeyValue(node.opacityProperty(), node.getOpacity(), WEB_EASE)),
                    new KeyFrame(Duration.millis(time),  new KeyValue(node.opacityProperty(), 1, WEB_EASE))
                )
                .build()
            );
