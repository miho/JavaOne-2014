...
public class MarchingSquares_double {

    private final Data_double forceField;
    ...
    public MarchingSquares_double(Data_double forceField) {
    ...
    public Path_double computePaths(double isoVal) {
        Path_double result = new Path_double(isoVal);
        ...
        for (int x = 0; x < w; x++) {
            for (int y = 0; y < h; y++) {
                                                ...
                                                if (x < 1) {
                    forceField.set((double)0, x, y);
                    forceField.set((double)0, x, y + 1);
                }

                if (y < 1) {
                    forceField.set((double)0, x, y);
                    forceField.set((double)0, x + 1, y);
                }

                if (x == w - 1) {
                    forceField.set((double)0, x + 1, y);
                    forceField.set((double)0, x + 1, y + 1);
                }
                ...
                final double val0 = forceField.get(x, y);
                final double val1 = forceField.get(x + 1, y);
                final double val2 = forceField.get(x + 1, y + 1);
                final double val3 = forceField.get(x, y + 1);
    ...
    private void interpolatePoint(double isoVal,
            int x, int y,
            Vector2d result,
            int edgeIndex) {
        ...
        final double valueA = forceField.get(p1X,p1Y);
        final double valueB = forceField.get(p2X,p2Y);
